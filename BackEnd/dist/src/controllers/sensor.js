...
export const listSensors = async (req, res) => {
    try {
        ...
        if (!house)
            return res.status(404).json({ error: "ไม่พบข้อมูลบ้าน หรือคุณไม่มีสิทธิ์เข้าถึง" });
        const sensors = await prisma.sensor.findMany({
            where: { houseId: houseId }
        });
        res.status(200).json({
            sensors,
            ownerName: house.user.fullName,
            isPushEnabled: house.user.pushNotify
        });
    }
    catch (error) {
        console.log(error);
...
export const createSensor = async (req, res) => {
    try {
        ...
        if (!house)
            return res.status(403).json({ error: "คุณไม่มีสิทธิ์เพิ่มเซนเซอร์ในบ้านหลังนี้" });
        const existingSensor = await prisma.sensor.findUnique({
            where: { serialNumber: String(serialNumber) }
        });
        if (existingSensor)
            return res.status(400).json({ error: "Sensor ID นี้ถูกเพิ่มในระบบแล้ว" });
